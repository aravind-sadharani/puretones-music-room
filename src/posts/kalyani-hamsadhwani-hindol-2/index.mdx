...
* In the first two phrases, one can see Ga, Ni' and Sa being prominently used. In phrases like `(Ni',Re,Ga)` and `(Ni',Re,Pa,Ma,Ga)` it would be aesthetically pleasing if Ni' and Ga have a consonant relationship. In phrases `(Sa,Ni',Re,Ga)` and `(Sa,Ni',Re,Pa,Ma,Ga)` it would be good to have a consonant relationship between Sa and Ga.

* In this version, since `(Re,Ma)` is not a consonant relationship, it would be best to avoid phrases which emphasise the pairing of these notes.

* Phrase 3 establishes a symmetric relationship between the intervals `(Ga,Pa)` and `(Ma,Dha)`. 

* Phrases 4 and 5 establishes consonant relationship between `(Pa,Ni)` and `(Ga,Ni)`.

It is important to note that in addition to the phrases, it is equally important to pay attention to the timing of notes in a phrase, and the *Alpatva* *Bahutva* *Hrasva* *Deergha* [characteristics](/posts/grammar-of-music) of the notes in the phrase to determine the appropriate pairings.
...
export const kalyaniMotif = `Ga Re Sa 2 Ni' Re Ga 2 Ma Ga Pa Ma Dha Pa 2 Ni 2 Dha Pa Ma Dha Ni SA 2
...
Note: Some musicians/musicologists classify Raga Hamsadhwani under Raga Shankarabharanam. The scale of Shankarabharanam is Sa Re Ga ma Pa Dha Ni. Hamsadhwani can be derived by ma Dha Varjit.

## Simple Phrases in Raga Hamsadhwani

Here are some simple phrases to understand the scale and structure of Raga Hamsadhwani.

export const hamsadhwaniMotif = `Ga Re Sa Re Ga 2 Ga Re Ga Pa Ni 2 SA 2
...
Raga Hindol is widely accepted to be related to Raga Kalyani. Indeed, it can be arrived at by dropping Re and Pa from Kalyani. Raga Hindol uses the symmetry `(Ma,Dha) = (Dha,SA)` and `(Sa,Ga)` = *Antara Gandhara Bhav* interval. Again, if you observe closely, you can see that these relationships are already present in Raga Kalyani.

Some characteristic phrases of Raga Hindol using these *Lakshanas* would be `Sa Ga Ma Ga SA` and `SA Ni Dha Ma Ga`.

## Simple Phrases in Raga Hindol

Here are some simple phrases to understand the scale and structure of Raga Hindol.

export const hindolMotif = `Sa Ga Sa 2 Ni' Dha' Sa 2 Ga Ma Dha 2 Ma Dha SA 2
